...
public:
	virtual HRESULT Initialize_Prototype(TYPE eModelType, const char* pModelFilePath, _fmatrix PivotMatrix, _bool bForceNormals);
...
private:
	...
	HRESULT Create_Materials(const char* pModelFilePath, _bool bForceNormals);
	HRESULT Create_HierarchyNodes(const aiNode* pNode, CHierarchyNode* pParent = nullptr);
	HRESULT Create_Animations();

public:
	static CModel* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, TYPE eModelType, const char* pModelFilePath, _bool bForceNormals = true, _fmatrix PivotMatrix = XMMatrixIdentity());
